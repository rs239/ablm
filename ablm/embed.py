...
def compute_prob(sequence, embed_type = 'beplerberger', embed_device = None, embed_model = None):
    if embed_device is None:
        device = torch.device(base_config.device if torch.cuda.is_available() else "cpu")
    else:
        device = torch.device(embed_device)
    ...
    if embed_type == 'beplerberger':
        with torch.no_grad():
            ...
            x = x.cuda(device)
            if embed_model is None:
                a = bb_model.forward(x)
            else:
                a = embed_model.forward(x)
            loss = -a.detach().sum().cpu() / len(sequence)
        ...
        out_temp = results["logits"][:,1:-1,:]

        loss = -out_temp.detach().cpu().sum() / len(sequence)
