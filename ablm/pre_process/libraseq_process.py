...
sys.path.append('/net/scratch3/scratch3-3/chihoim/antibody_ml')
...
def create_standardized_dataset():
    ...
    libra_data_path = '/data/cb/rsingh/work/antibody/ci_data/raw/libraseq'
    ...
    with open(os.path.join(libra_data_path, 'libraseq_standardized.csv'), 'w') as f:
        writer = csv.writer(f)
        ...
        for df, name in [(df1, 'cell-lines'), (df2, 'd45'), (df3, 'n90')]:
            ...
            for index, row in tqdm(df.iterrows(), total=df.shape[0]):
                ...
                if vh_seq != '':
                    ...
                    os.system('/data/cb/rsingh/tools/bin/transeq -sequence {} -outseq {} -trim True -clean True'.format(gene_path, pep_path))
...
def divide_sets():
    libra_path = '/data/cb/rsingh/work/antibody/ci_data/raw/libraseq'

    df = pd.read_csv(os.path.join(libra_path, 'libraseq_standardized.csv'), keep_default_na=False)
...
def sample_from_allpairs():
    ...
    samp_h = df_h.sample(n=100000)
    samp_l = df_l.sample(n=100000)

    save_dir = '/data/cb/rsingh/work/antibody/ci_data/processed/libraseq'
